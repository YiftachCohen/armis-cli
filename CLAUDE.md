...
- `internal/progress/` - Braille-dot spinner with timer display, width-aware message truncation, lipgloss styling, cursor hiding, CI detection (auto-disables animation). Context-aware with configurable timeout (default 30min). Upload progress via `NewReader()`/`NewWriter()` wrappers.
