...
- `internal/progress/` - The scan experience renderer (see `docs/design/scan-experience-spec.md`). `Spinner` (classic braille head @80ms, breathing ellipsis, timer) for simple commands; `Phase` (StartPhase/SetDetail/StreamLine/SetVerbs/Succeed) renders the scan's live region — status line + stream sub-lines with cursor-up redraws — collapsing into append-only ✓ receipt trails. `RenderFinale` plays the Armis arrow one-shot (braille dot assemble + light sweep) and the staggered findings reveal / severity count-up. `osc.go` holds gated terminal integrations (OSC 8 hyperlinks via `ARMIS_CONSOLE_URL`, tab title, Windows Terminal taskbar progress, per-terminal notifications). Everything degrades to plain printed lines in CI/non-TTY; width truncation guards the live region against soft-wrap corruption. Context-aware with configurable timeout (default 30min). Upload progress via `NewReader()`/`NewWriter()` wrappers.
