...
- `internal/progress/` - Animated loader: braille sine-wave strip with brand-violet gradient (amplitude → shade) plus a shimmer highlight sweeping the message text, timer display, width-aware message truncation, lipgloss styling, cursor hiding, CI detection (auto-disables animation). Context-aware with configurable timeout (default 30min). Upload progress via `NewReader()`/`NewWriter()` wrappers.
